---
name: test-runner
description: Use this agent proactively after any code changes to run tests and fix failures. Also invoke when tests are mentioned in conversation, when implementing new features, after refactoring, or when the user asks about test status or code quality.\n\nExamples:\n\n<example>\nContext: User just finished implementing a new feature\nuser: "Add a function to calculate compound interest with principal, rate, and time parameters"\nassistant: "Here's the compound interest function:"\n<function implementation completed>\nassistant: "Now let me use the test-runner agent to verify the code works correctly and all tests pass"\n<Task tool invocation with test-runner agent>\n</example>\n\n<example>\nContext: User mentions tests in conversation\nuser: "Are the tests passing?"\nassistant: "Let me use the test-runner agent to check the test status"\n<Task tool invocation with test-runner agent>\n</example>\n\n<example>\nContext: User just refactored existing code\nuser: "Refactor the user authentication module to use async/await instead of callbacks"\nassistant: "I've refactored the authentication module to use async/await:"\n<refactoring completed>\nassistant: "Now I'll invoke the test-runner agent to ensure the refactoring didn't break any existing functionality"\n<Task tool invocation with test-runner agent>\n</example>\n\n<example>\nContext: User fixed a bug\nuser: "Fix the off-by-one error in the pagination logic"\nassistant: "I've fixed the off-by-one error:"\n<bug fix completed>\nassistant: "Let me run the test-runner agent to verify the fix and ensure no regressions"\n<Task tool invocation with test-runner agent>\n</example>
model: opus
color: yellow
---
